...
with st.expander("ℹ️ Click to learn how this simulation works"):
    ...
    - A LiFePO4 Battery (EFOY Li 105)

    The goal is to estimate energy autonomy and methanol consumption for two seasonal use profiles:
    - ☀️ Summer (low lighting/heating needs)
    - ❄️ Winter (longer usage, heating control active)

    All values are simulated for educational purposes.
    """)

st.sidebar.header("🧰 Customize your devices")
season = st.sidebar.radio("Select Season", ["☀️ Summer", "❄️ Winter"], horizontal=True)

default_appliances = summer_appliances if "Summer" in season else winter_appliances
custom_appliances = []
...
if st.button("📄 Generate PDF Report"):
    ...
    pdf.cell(200, 10, txt="🔋 EFOY Hybrid Power System Report", ln=True, align='C')
    ...
    for index, row in summary_df.iterrows():
        line = f"- {row['name']}: {row['power']} W x {row['hours']} h = {row['Energy (Wh)']:.0f} Wh"
    ...
    pdf.multi_cell(0, 10, txt="Report generated for educational purposes - Task 2: Camping Truck. Servus! 🛻")
